...
int main (int argc, char* argv[]){
    ...
    //print intro
    printf("Soubor: %s, sloupce: %zu, radky: %zu\n", argv[2], myList.nCols, myList.nRows - 1); //prvni radek je template sloupcu vzdy
    ...
    while (true) {
        ...
        stripped_cmd_argv = (char**)malloc(sizeof(char*) * (commandArgc + 2)); //argc ukazuje plus jedna pac prvni radek nekopiruju, takze pro null uz mam misto
		...
		if (my_strcmp(command, "exit")){
            ...
            if (!fclose(fout)){
                printf("output file was not closed right, aborting...\n");
                exit_seq(&myList, command, parsed_command, stripped_cmd_argv, inputBase, f);
                return 1;
            }
            */
