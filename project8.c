...
void dyn_copy2D(const char** parsedBuffer, char** dest){
...
// 1 a cekuj ho ve while jako prvni at uz se pak nevola ta fce co by zapsala za pamet ...flag && read_dyn_FILE....                                                            
    ...
    while (true) {
       /* if (command)
        ...
        stripped_cmd_argv = (char**)malloc(/*stripped_cmd_argv,*/ sizeof(char*) * (commandArgc + 1)); //argc ukazuje plus jedna pac prvni radek nekopiruju, takze pro null uz mam misto
		...
		switch (determin_command(parsed_command[0])){
			case 0:
                ...
                /*stripped_cmd_argv =*/ dyn_copy2D((const char**)parsed_command, stripped_cmd_argv);               
				free_2D(parsed_command);
